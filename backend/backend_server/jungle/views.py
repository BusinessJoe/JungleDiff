...
def summoner_matches(request):
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                save_summoner_match_history(summoner_name)
            except HTTPError as err:
                # Replace requests HTTPError with equivalent Django HttpResponse
                return HttpResponse(status=err.response.status_code)
            return HttpResponse()
    return HttpResponseNotAllowed(['POST'])


def save_summoner_match_history(summoner_name):
    """Save the summoner's most recent 5 ranked solo games to the database"""
